...
## 1 | Menyisipkan Elemen ke dalam Multimap
...
## 2 | Menghapus Elemen dari Multimap
...
---
### Menghapus hanya Beberapa elemen dengan key sama
> Dijelaskan bahwa kita bisa menghpaus elemen mulitmap dengan key nya, tapi bagaimana jika aku ingin menghapus suatu elemen di mulitmap, yang walaupun key nya sama, tapi jika value nya tidak memnuhi kondisi tertentu, maka key dan value tersebut dihapus?
...
## 3 | Mengecek Ukuran dan Kekosongan Multimap

Kita dapat memeriksa apakah sebuah **multimap** kosong dengan menggunakan metode `empty()` dan menghitung ukurannya dengan metode `size()`.

Metode `empty()` akan mengembalikan:

- `true` – jika **multimap** kosong.
    
- `false` – jika **multimap** tidak kosong.
    

Sedangkan, metode `size()` akan mengembalikan jumlah elemen di dalam **multimap**. Sebagai contoh:
...
Di sini, dengan menggunakan **operator ternary**, variabel `result` menyimpan string:

- `"Yes"` jika **multimap** kosong (yaitu, ketika `empty()` mengembalikan `true`)
    
- `"No"` jika **multimap** tidak kosong (yaitu, ketika `empty()` mengembalikan `false`)

---

## 4 | Mencari Key dalam Multimap

**Mencari Sebuah Kunci di Multimap**  
Kita dapat menggunakan fungsi `find()` untuk mencari elemen dengan kunci tertentu.

Fungsi `find()` akan mengembalikan:

- **Iterator ke elemen** jika kunci ditemukan
    
- **Iterator ke akhir kontainer** (yaitu `end()`) jika kunci tidak ditemukan
...
Seperti yang terlihat pada contoh, metode `find()` hanya mengembalikan **kemunculan pertama** dari kunci tersebut dalam **multimap**.
...
## 5 | Mengecek Apakah Key Ada dalam Multimap

Kita dapat menggunakan metode berikut untuk memeriksa apakah kunci tertentu ada di dalam **multimap**:

- `find()` – mengembalikan iterator ke elemen jika kunci ditemukan, jika tidak maka mengembalikan iterator `end()`
    
- `count()` – mengembalikan jumlah total kemunculan kunci yang ditentukan
...
Pada contoh di atas, kita telah menggunakan metode `find()` dan `count()` untuk memeriksa apakah suatu kunci terdapat dalam **multimap**.

Pertama, kita menggunakan metode `count()` untuk mengetahui berapa kali kunci `20` muncul:

```cpp
int count_20 = students.count(20);
```

Lalu, kita menggunakan metode `find()` untuk memeriksa apakah kunci `27` ada:
